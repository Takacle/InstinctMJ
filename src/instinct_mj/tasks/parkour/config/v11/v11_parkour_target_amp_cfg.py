"""V11 parkour AMP task config factories.

Adapted from G1 parkour config. Key differences vs G1:
- Root body: base_link (not pelvis/torso_link)
- 29-DOF full-body model: legs (0-11), waist (12-14), left arm (15-21), right arm (22-28)
- No shoe variant
- Camera mounted at waist_pitch_link
- Motion reference uses V11 XML (v11_v4.xml) and 29-DOF symmetric mappings
- AMP discriminator operates on full 29-DOF joint state (joint_names=".*")
"""
...
class AmassMotionCfg(AmassMotionCfgBase):
    """Parkour AMASS motion buffer config for V11."""
...
motion_reference_cfg = MotionReferenceManagerCfg(
    ...
    # base_link and waist_pitch_link are midline; the rest are left↔right pairs
    symmetric_augmentation_link_mapping=[0, 1, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12],
    symmetric_augmentation_joint_mapping=list(V11_29Dof_symmetric_augmentation_joint_mapping),
    symmetric_augmentation_joint_reverse_buf=list(V11_29Dof_symmetric_augmentation_joint_reverse_buf),
...
def instinct_v11_parkour_amp_env_cfg(
    *,
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Build the V11 parkour AMP environment configuration.
    ...
    v11_robot_cfg.articulation.actuators = copy.deepcopy(beyondmimic_v11_wholebody_delayed_actuator_cfgs)
    ...
    # Scene sensors — V11 uses base_link for torso contact, waist_pitch_link for camera
    cfg.scene.sensors = (
        ContactSensorCfg(
            name="contact_forces",
    ...
    amp_reference_terms = {
        ...
        "joint_pos_rel": ObservationTermCfg(
            ...
            params={"asset_cfg": SceneEntityCfg(name="motion_reference", joint_names=".*")},
            history_length=10,
            flatten_history_dim=True,
        ),
        "joint_vel": ObservationTermCfg(
    ...
    cfg.rewards = {
        ...
        # V11: root is base_link (not pelvis)
