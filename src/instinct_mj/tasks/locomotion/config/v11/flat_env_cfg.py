"""mjlab-native V11 locomotion environment config builder.

V11 uses a 12-DOF legs-only MJCF model; upper-body bodies are present for
collision geometry but all waist/arm joints are fixed (welded).
Root body: base_link.
"""
...
V11_CFG = V11_29DOF_POPSICLE_CFG
...
def _scene_cfg(play: bool) -> SceneCfg:
    robot_cfg = copy.deepcopy(V11_CFG)
    robot_cfg.articulation.actuators = copy.deepcopy(beyondmimic_v11_locomotion_actuator_cfgs)
...
def _rewards_cfg() -> dict[str, RewTerm]:
    return {
        ...
        "stand_still": RewTerm(
            func=locomotion_mdp.stand_still,
            weight=-0.8,
...
def instinct_v11_locomotion_flat_env_cfg(*, play: bool = False) -> ManagerBasedRlEnvCfg:
    """Build V11 flat-ground locomotion config in mjlab-native manager dict style."""
